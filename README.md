...
## Описание
EDS_Store - интернет магазин на платформе iOS, так как проект посвящен компании EDS-Privid, тематика приложения и цветовые решения были выбраны основываясь на дизайн компании и их деятельность
...
### 1) Склонируйте прроект 
Code -> Copy -> откройте командную строку и склонируйте репозиторий с помощью git clone

### 2) Откройте проект и нажмите Run 

### 3) У вас должны будут скачаться зависимости (FireBase) 
...
### Техническая часть
-- **Экран корзина:** получает данные из UserDefaults, если их нет просит добавить их, после загрузки данных, BagModel обращается к методу NetworkLayer для получения изображении по url. Кнопка оформление переносит пользователя на экран Оформленние заказа
-- **Экран Офомление заказа:** аналогично получает фото из интернета, если адресс и способ оплаты указан, то при нажатии заказать, корзина очищается пользователь видет сообщение об успешном оформлении заказа. Под капотом: Создается новая запись на Firebase Database о совершенном заказе

---

## Дополнительные скрины экранов

<img src="assets/images/sex.jpg" alt="Пример работы приложения" width="300" height="650">

<img src="assets/images/emptyBag.PNG" alt="Пример работы приложения" width="300" height="650">

<img src="assets/images/emptyUser.PNG" alt="Пример работы приложения" width="300" height="650">
